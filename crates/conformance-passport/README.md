...
`make check`: anchor drift or a hand edit of the JSON fails CI. Because the seal is
...
`{"kind":"service","id":…}` cleartext. No P-scenario exercises it over HTTP: the Go
anchor resolves both actor kinds to a `Passport::Human`, and asserting that would
freeze a resolution policy a real `svc-identity` may legitimately choose otherwise.
`vectors_test.go` opens it and checks the cleartext; the battery does not.
