...
pub fn subject_dir() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../conformance-subjects/identity-passport")
        .canonicalize()
        .expect("identity-passport subject directory must exist")
}
...
pub async fn run_retired_model_guard(dir: &Path) -> Result<()> {
    ...
    if !output.status.success() {
        return Err(ConformanceError::Build(format!(
            "retired-model guard failed for {} (status {}):\n{}\n{}",
            dir.display(),
            output.status,
...
pub async fn build_subject() -> Result<PathBuf> {
    ...
    let dir = subject_dir();
    run_retired_model_guard(&dir).await?;
    let dir_str = dir.to_string_lossy().into_owned();
