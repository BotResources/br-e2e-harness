...
- `DeclareCapture` — a subscribe-**first** ephemeral consumer
  (`DeliverPolicy::New`, `AckPolicy::None`) over the declare subject that drains
  declares into a shared buffer; `decode()` is the type-oracle check.
...
crates.io). Keep its `br-rust-common` rev identical to `br-test-harness`'s
(`70db51f`) so Cargo resolves a single source and never duplicates `br-core-*`:

```toml
[dev-dependencies]
conformance-scope = { git = "ssh://git@github.com/BotResources/br-e2e-harness", tag = "conformance-scope-v0.1.0" }
```
...
| `DeclareCapture` subscribes **before** the subject is spawned | The declarer's awaiter uses `DeliverPolicy::New`; a confirmation pre-published before its consumer exists is missed. Symmetrically, a declare published before the capture consumer exists would be missed — so capture is armed first, then the subject boots. |
