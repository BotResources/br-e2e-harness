...
| **C6** | a registered `ImpactStager` is called **on the sink's own connection, inside the roster transaction**: a committed roster write and its impacts are durable together (the stager reads the still-uncommitted roster row through the `conn` it is handed), a stager that **refuses** one key rolls that key's roster write back and leaves the pre-existing row untouched while a lower-ordered sibling key stays converged, each roster write stages **exactly** the entities it makes unnameable afterwards, a converged mirror stages nothing, and a projector with **no** stager registered converges the roster while staging nothing at all. |
...
- `stager_stages_in_the_projection_transaction(&snapshot)` — the C6 stager
  ...
  was already visible to that connection. `StagedImpact` is the read-back row.
